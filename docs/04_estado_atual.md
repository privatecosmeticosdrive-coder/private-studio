...
- **Próxima FASE da Jornada de Laboratório:** (4) **indicadores** de prazo/atraso e tempos médios solicitação→conclusão **por urgência** — D5 já grava todos os timestamps (`solicitada_em`, `atendida_em`, `concluida_em`, `prazo_limite`); falta a tela. Lembrete: **atraso é DERIVADO na leitura, nunca coluna** (coluna persistida mente no dia seguinte).
