...
- **🔴 Busca de fórmulas sem match mostra sequência-padrão como se fosse resultado** (Shampoo Organika, Sérum Facial Dia, …): relatado 3x pelo Gabriel — quebra credibilidade da ferramenta. Separar **match real de fallback** na lógica de match/ranking; quando não houver match, **estado vazio honesto** ("nenhuma fórmula corresponde ao briefing"). Frente separada (não é UI só: mexe no ranking).
- **Renomear menu "Cotações" → "Cotações externas"** (desambiguar de Orçamentos). Fatia rápida (label + rota).
- **Motivo de reprovação no orçamento** *(desenho do Gabriel, red team #50)*: ao reprovar, registrar motivo categorizado (preço/custo, fórmula, prazo, outro + texto). Motivo=preço → abre margem investigativa; motivo=fórmula → gera NOVA VERSÃO da fórmula com composição ajustada, rastreando o orçamento original. Só na reprovação.
...
- **194 fórmulas sem NCM (resto manual):** ver a dívida "Atribuição de NCM" — 346/540 já atribuídas em lote assistido; o resto é sob demanda + o #436.
