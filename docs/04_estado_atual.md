...
- **Próximas FASES da Jornada de Laboratório:** (2) gatilhos de **revisão de fórmula selecionada** + **melhoria proativa** (lab abre revisão a qualquer momento); (3) **motivo de reprovação categorizado** (preço/custo, fórmula, prazo, outro + texto) + gatilho automático `custo_formula`→pendência de revisão (hook comentado em `status-orcamento.util.ts` e `mudar-status.dto.ts`); (4) **indicadores** de prazo/atraso e tempos médios solicitação→conclusão por urgência (D5 já grava todos os timestamps).
